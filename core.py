...
DEFAULT_SETTINGS = {
    ...
    "withdraw_qr_enabled": True,
    "withdraw_qr_payee_name": "UPI Loot Pay",
}
...
bot = telebot.TeleBot(BOT_TOKEN, parse_mode=None, threaded=True, num_threads=16)
...
SETTINGS_CACHE = {}
SETTINGS_CACHE_LOCK = threading.RLock()
SETTINGS_CACHE_TTL_SECONDS = 10
...
def init_db():
    ...
    c.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS withdrawals (
            ...
            txn_id TEXT DEFAULT '',
            bonus_taxable_amount REAL DEFAULT 0
        );
    ...
    try:
        c.execute("ALTER TABLE withdrawals ADD COLUMN bonus_taxable_amount REAL DEFAULT 0")
    except:
        pass
...
def get_setting(key):
    ...
    with SETTINGS_CACHE_LOCK:
        ...
        if cached and (now_ts - cached[0]) <= SETTINGS_CACHE_TTL_SECONDS:
            return cached[1]
    ...
    with SETTINGS_CACHE_LOCK:
        SETTINGS_CACHE[key] = (now_ts, value)
    return value
...
def set_setting(key, value):
    ...
    if key == "max_withdraw_per_day":
        ...
        clear_settings_cache("max_single_withdraw_amount")
    elif key == "max_single_withdraw_amount":
        db_execute(
            "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
            ("max_withdraw_per_day", json.dumps(value))
        )
        clear_settings_cache("max_withdraw_per_day")
...
def get_withdrawal_tax_breakdown(user, amount):
    ...
    net_amount = round(max(0.0, amount - total_tax), 2)
    return {
        ...
        # GST/tax is deducted from the withdrawal amount, not added extra.
        # Example: ₹10 withdrawal with 10% GST = ₹10 wallet debit and ₹9 payout.
        "total_debit": round(amount, 2),
    }


def build_upi_payment_link(upi_id, amount, note="Withdrawal By @realUpiLootBot", payee_name=None):
    amount = round(float(amount or 0), 2)
    params = {
        "pa": str(upi_id or "").strip(),
        "pn": str(payee_name or get_setting("withdraw_qr_payee_name") or "UPI Loot Pay").strip(),
        "am": f"{amount:.2f}",
        "cu": "INR",
...
def make_upi_qr_file(upi_id, amount, note="Withdrawal By @realUpiLootBot"):
    if qrcode is None:
        return None
    upi_link = build_upi_payment_link(upi_id, amount, note)
...
def send_withdrawal_qr_to_admins(withdrawal_id, user, upi_id, payout_amount, requested_amount=0, gst_amount=0, force=False):
    if not force and not bool(get_setting("withdraw_qr_enabled")):
        return
    caption = (
        f"{pe('link')} <b>UPI Payment QR</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{pe('info')} <b>WD ID:</b> #{withdrawal_id}\n"
        f"{pe('disguise')} <b>User:</b> {h(user['first_name'] if user else 'Unknown')} (<code>{user['user_id'] if user else ''}</code>)\n"
        f"{pe('fly_money')} <b>Withdrawal:</b> ₹{float(requested_amount or 0):.2f}\n"
        f"{pe('receipt')} <b>GST/Tax:</b> ₹{float(gst_amount or 0):.2f}\n"
        f"{pe('money')} <b>Pay Amount:</b> ₹{float(payout_amount or 0):.2f}\n"
        f"{pe('link')} <b>UPI:</b> <code>{h(upi_id)}</code>\n\n"
        f"Scan this QR to pay the user.\n"
        f"━━━━━━━━━━━━━━━━━━━━━━"
    )
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("✅ Approve", callback_data=f"apprv|{withdrawal_id}"),
        types.InlineKeyboardButton("❌ Reject", callback_data=f"rejct|{withdrawal_id}"),
    )
    admin_ids = []
    for admin in get_all_admins():
        try:
            admin_ids.append(int(admin['user_id']))
        except Exception:
            pass
    ...
    for admin_id in sorted(set(admin_ids)):
        try:
            # Create a fresh in-memory QR file for every admin. Some Telegram clients
            # close/read file-like objects after upload, so reusing one BytesIO can
            # randomly fail for second/extra admins.
            qr_file = make_upi_qr_file(upi_id, payout_amount, "Withdrawal By @realUpiLootBot")
            if qr_file is not None:
                bot.send_photo(admin_id, qr_file, caption=caption, parse_mode="HTML", reply_markup=markup)
            else:
                upi_link = build_upi_payment_link(upi_id, payout_amount, "Withdrawal By @realUpiLootBot")
                safe_send(admin_id, caption + f"\n\n<code>{upi_link}</code>", reply_markup=markup)
        except Exception as e:
            print(f"Admin QR notify error to {admin_id}: {e}")
