...
def universal_handler(message):
    ...
    if state == "enter_amount":
        ...
        if tax["total_debit"] > user["balance"]:
            safe_send(message.chat.id, f"{pe('cross')} Insufficient balance! Need ₹{tax['total_debit']:.2f} balance. You have ₹{user['balance']:.2f}")
            return
        ...
        safe_send(
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"{pe('fly_money')} <b>Withdrawal Amount:</b> ₹{amount:.2f}\n"
            f"{pe('receipt')} <b>GST/Tax Cut:</b> ₹{tax['total_tax']:.2f}\n"
            f"{pe('money')} <b>You Will Receive:</b> ₹{tax['net_amount']:.2f}\n"
    ...
    if state == "admin_set_withdraw_qr_payee":
        name = text.strip()
        if not name:
            safe_send(message.chat.id, f"{pe('cross')} Payee name cannot be empty.")
            return
        clear_state(user_id)
        set_setting("withdraw_qr_payee_name", name[:60])
        safe_send(message.chat.id, f"{pe('check')} QR payee name updated to: <b>{h(name[:60])}</b>")
        return
