...
def admin_withdrawals(message):
    ...
    qr_enabled = bool(get_setting("withdraw_qr_enabled"))
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
    ...
    markup.add(
        types.InlineKeyboardButton(f"{'🟢' if qr_enabled else '🔴'} Auto QR", callback_data="tog_withdraw_qr"),
        types.InlineKeyboardButton("✏️ QR Payee Name", callback_data="s_withdraw_qr_payee"),
    )
    safe_send(
        message.chat.id,
        ...
        f"{pe('link')} <b>Auto QR:</b> {'ON' if qr_enabled else 'OFF'}",
        reply_markup=markup
    )


@bot.callback_query_handler(func=lambda call: call.data == "tog_withdraw_qr")
def tog_withdraw_qr(call):
    if not is_admin(call.from_user.id):
        return
    cur = bool(get_setting("withdraw_qr_enabled"))
    set_setting("withdraw_qr_enabled", not cur)
    safe_answer(call, f"Auto QR {'Enabled' if not cur else 'Disabled'}!")
    class _Msg:
        chat = call.message.chat
    admin_withdrawals(_Msg())


@bot.callback_query_handler(func=lambda call: call.data == "s_withdraw_qr_payee")
def s_withdraw_qr_payee(call):
    settings_ask(call, "admin_set_withdraw_qr_payee", f"{pe('pencil')} Enter QR payee name shown in UPI apps:")


@bot.callback_query_handler(func=lambda call: call.data.startswith("sendqr|"))
def send_withdraw_qr_manual(call):
    if not is_admin(call.from_user.id):
        return
    try:
        w_id = int(call.data.split("|", 1)[1])
    except Exception:
        safe_answer(call, "Invalid QR request!", True)
        return
    wd = db_execute("SELECT * FROM withdrawals WHERE id=?", (w_id,), fetchone=True)
    if not wd:
        safe_answer(call, "Withdrawal not found!", True)
        return
    u = get_user(wd["user_id"])
    payout_amt = float(wd["net_amount"] or wd["amount"] or 0)
    gst_amt = float(wd["gst_amount"] or 0)
    send_withdrawal_qr_to_admins(w_id, u, wd["upi_id"], payout_amt, wd["amount"], gst_amt, force=True)
    safe_answer(call, "QR sent to admins!")
...
def wdlist_pending(call):
    ...
    for w in rows:
        ...
        payout_amt = float(w["net_amount"] or w["amount"] or 0)
        markup.add(types.InlineKeyboardButton("👤 Info", callback_data=f"uinfo|{w['user_id']}"))
        markup.add(types.InlineKeyboardButton("📷 Send QR", callback_data=f"sendqr|{w['id']}"))
        safe_send(
            call.message.chat.id,
            f"{pe('hourglass')} <b>Pending #{w['id']}</b>\n\n"
            f"{pe('disguise')} {name} (<code>{w['user_id']}</code>)\n"
            f"{pe('fly_money')} Withdrawal: ₹{float(w['amount'] or 0):.2f}\n"
            f"{pe('money')} Pay: ₹{payout_amt:.2f}\n"
...
def show_settings(chat_id):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton(f"{'🟢' if bool(get_setting('withdraw_qr_enabled')) else '🔴'} Auto QR", callback_data="tog_withdraw_qr"),
    )
    markup.add(
        types.InlineKeyboardButton("✏️ QR Payee Name", callback_data="s_withdraw_qr_payee"),
